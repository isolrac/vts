...
| MCU Pin | Function |
|---|---|
| D2 | PWM output → DRV8833 AIN1, AIN2, BIN1, BIN2 (both motors, same speed) |
| D0 | ADC input ← voltage divider (battery sense) |
| 3V3 | DRV8833 nSLEEP (always enabled) |
...
- Voltage divider scales 3.0–4.2V battery range to within the 3.3V ADC input range
- Measured voltage formula: `battery_voltage = adc_reading × (3.3 / 65535) × ((R1 + R2) / R2)`

**LDO dropout compensation**
- The MCP1700 regulates cleanly above ~3.18V (3.0V output + 178mV dropout)
- Below 3.18V, the output tracks input minus dropout; motors would slow without correction
- Firmware compensates automatically: `effective_duty = target_duty × (3.0 / battery_voltage)`

**Hard motor cutoff**
- Motors are disabled entirely at 3.0V battery voltage
- Prevents deep discharge damage to the LiPo

**Maximum duty cycle**
- Capped at ~71% (3.0 / 4.2) at full battery charge — 3.0V is the nominal motor voltage per the datasheet
